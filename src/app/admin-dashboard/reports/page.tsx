...
const QUOTATION_STATUS_CHIPS: Record<string, string> = {
  ...
  order_confirmed: 'bg-[#1877F2]/10 text-[#1877F2]',
...
const FULFILLMENT_CHIPS: Record<string, string> = {
  ...
  delivered: 'bg-[#1877F2]/10 text-[#1877F2]',
  cancelled: 'bg-red-50 text-red-600',
}
  ...
  return (
    <div className="py-3.5 border-b border-gray-50 last:border-0 w-full overflow-hidden">
      ...
      <div className="w-full h-1.5 sm:h-2 bg-gray-100 rounded-full overflow-hidden">
        <div className="h-full bg-[#1877F2] rounded-full transition-all duration-500" style={{ width: `${pct}%` }} />
      </div>
    </div>
  )
}
    ...
    <div className="w-full max-w-[1000px] mx-auto antialiased overflow-x-hidden pb-10">
      ...
      <div className="bg-[#1d1d1f] text-white rounded-[1.5rem] sm:rounded-[2rem] p-5 sm:p-6 md:p-8 mb-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 md:gap-8 shadow-lg w-full overflow-hidden">
        <div className="w-full min-w-0 md:w-auto">
          ...
          <p className="text-[32px] sm:text-[36px] md:text-[48px] font-semibold tracking-tight leading-none text-[#1877F2] break-words">
            {peso(totalNetProfit)}
          </p>
        ...
        <div className="flex flex-col gap-1.5 text-[12px] sm:text-[13px] text-gray-400 font-medium w-full md:w-auto min-w-0 md:text-right">
           ...
           <p className="flex justify-between md:justify-end gap-4 sm:gap-6 text-[#1877F2] font-semibold"><span>Net Profit:</span> <span className="truncate">{peso(totalNetProfit)}</span></p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5 mb-8 w-full">
        ...
        <div className="bg-[#1877F2]/5 border border-[#1877F2]/20 rounded-[1.5rem] p-5 shadow-sm flex flex-col overflow-hidden w-full">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-[#1877F2]/80 mb-1 truncate">Amount Paid</p>
          <p className="text-[20px] sm:text-[24px] font-semibold tracking-tight text-[#1877F2] mb-3 truncate">{peso(totalAmountPaid)}</p>
          <div className="mt-auto">
            <p className="text-[11px] text-[#1877F2]/80 font-medium truncate">Full & partial payments</p>
          </div>
        </div>
        ...
        <div className="bg-[#1d1d1f] rounded-[1.5rem] p-5 shadow-lg flex flex-col text-white overflow-hidden w-full">
          ...
          <p className="text-[20px] sm:text-[24px] font-semibold tracking-tight text-[#1877F2] mb-3 truncate">{peso(totalNetProfit)}</p>
          <div className="mt-auto">
        ...
        <div className={card}>
          ...
          {Object.entries(quotationPipeline).map(([status, v]) => (
            <BarRow
              ...
              chipClass={QUOTATION_STATUS_CHIPS[status] || 'bg-[#1877F2]/10 text-[#1877F2]'}
              count={v.count}
              value={v.value}
        ...
        <div className={card}>
          ...
          {Object.entries(orderPipeline).map(([status, v]) => (
            <BarRow
              ...
              chipClass={FULFILLMENT_CHIPS[status] || 'bg-[#1877F2]/10 text-[#1877F2]'}
              count={v.count}
              value={v.value}
              ...
              >
                <div className="flex-1 min-w-0 w-full">
                  ...
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    ...
                    <span className="text-[11px] font-medium text-[#1877F2] truncate">Paid: {peso(v.paid)}</span>
                ...
                <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center w-full sm:w-auto shrink-0 pt-2 sm:pt-0 mt-1 sm:mt-0 border-t sm:border-0 border-gray-100 min-w-0">
                  ...
                  <p className="text-[14px] font-semibold text-[#1877F2] font-mono leading-none truncate">
                    {peso(v.profit)}
                  </p>
