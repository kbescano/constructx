...
export default async function SalesReportPage({
  ...
  // "this month" until someone manually picks "All Time".
  ...
  return (
    <div className="w-full max-w-[900px] mx-auto antialiased pb-10">
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            <tr className="bg-white">
              ...
              <td className={`${cellClass} text-right font-mono text-[#1877F2] font-semibold`}>{peso(commission)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div className="sm:hidden border border-gray-200 rounded-xl overflow-hidden">
        <div className="bg-[#1877F2] text-white px-4 py-3 font-semibold text-[13px]">
          {myName || 'Unassigned'}
        </div>
        <div className="divide-y divide-gray-100 bg-white">
          ...
          <div className="flex items-center justify-between px-4 py-2.5 text-[12px] bg-gray-50">
            ...
            <span className="font-mono text-[#1877F2] font-semibold">{peso(commission)}</span>
          </div>
        </div>
      </div>
              ...
              <tbody>
                {sortedOrders.map(({ o, dateStr, oGross, oPaid, oAr, oCogs, oOpex, oProfit, oCommission }, i) => (
                  <tr key={o.id} className={`align-top ${i % 2 === 1 ? 'bg-gray-50' : 'bg-white'}`}>
                    <td className={`${detailCell} font-mono font-semibold text-[#050505]`}>{o.orderNumber || '--'}</td>
                    ...
                    <td className={`${detailCell} text-right font-mono text-[#1877F2] font-semibold whitespace-nowrap`}>{peso(oCommission)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-gray-100 border-t-2 border-[#050505]">
                  ...
                  <td className={`${detailCell} text-right font-mono font-bold text-[#1877F2] whitespace-nowrap`}>{peso(commission)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
          ...
          <div className="sm:hidden flex flex-col gap-3">
            {sortedOrders.map(({ o, dateStr, oGross, oPaid, oAr, oCogs, oOpex, oProfit, oCommission }) => (
              <div key={o.id} className="border border-gray-200 rounded-xl overflow-hidden">
                <div className="bg-[#050505] text-white px-4 py-2.5 flex items-center justify-between gap-2">
                ...
                <div className="divide-y divide-gray-100 bg-white">
                  ...
                  <div className="flex items-center justify-between px-4 py-2 text-[12px] bg-gray-50">
                    <span className="text-gray-700 font-semibold">Commission (20%)</span>
                    <span className="font-mono text-[#1877F2] font-semibold">{peso(oCommission)}</span>
                  </div>
                </div>
              </div>
            ))}

            <div className="border-2 border-[#050505] rounded-xl overflow-hidden">
              <div className="bg-[#050505] text-white px-4 py-2.5 font-bold text-[12px]">
                GRAND TOTAL
              </div>
              <div className="divide-y divide-gray-100 bg-gray-50">
                ...
                <div className="flex items-center justify-between px-4 py-2 text-[12px] font-bold">
                  ...
                  <span className="font-mono text-[#1877F2]">{peso(commission)}</span>
                </div>
              </div>
            </div>
          </div>
