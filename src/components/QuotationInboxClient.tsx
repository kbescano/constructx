          ...
          {rows.map(({ title, counts }) => (
            <tr key={title}>
              ...
              <td className="pl-2 py-0.5 text-center border-l border-gray-100">
                <span className="inline-flex items-center justify-center min-w-[22px] px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-[#050505]/5 text-[#050505]">
        ...
        {rows.map(({ title, counts }) => {
          ...
          return (
            <div key={title} className="flex flex-wrap items-center gap-1.5">
              ...
              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9.5px] font-bold whitespace-nowrap bg-[#050505]/5 text-[#050505]">
...
export default function QuotationInboxClient({
  ...
  return (
    <div className="w-full max-w-[900px] mx-auto py-6 overflow-x-hidden text-gray-700">
      ...
      {/* Search */}
      <OnboardingTip id="inbox-search" text="Search across every inquiry by customer name, phone, email, or item — try it with any of the seeded demo names." wrapperClassName="block w-full">
      ...
      {filteredRequests.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {filteredRequests.map((q: any, index: number) => {
            const card = (
              <div
            ...
            return (
              <div key={q.id}>
                {index === 0 ? (
                  <OnboardingTip id="inbox-card-click" text="Click any card to open the full request — details, status, and update notes all live in there." wrapperClassName="block w-full">
                    {card}
                  </OnboardingTip>
                ) : (
                  card
      ...
      <p className="mt-6 text-[11px] text-gray-400 text-center">
        ...
        <Link
          ...
          className="text-emerald-600 font-medium hover:text-[#050505] transition-colors underline underline-offset-2"
        >
          full CMS admin view
        </Link>
        .
