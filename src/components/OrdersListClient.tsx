            ...
            <div className="flex items-center gap-1.5">
              <span
                className={`w-2 h-2 rounded-full transition-colors ${
                  reached ? 'bg-[#1877F2]' : 'bg-gray-200'
                }`}
              />
              <span
                className={`text-[9px] font-bold uppercase tracking-wider ${
                  reached ? 'text-[#050505]' : 'text-gray-300'
                }`}
              >
                {FULFILLMENT_LABELS[stage]}
              </span>
            </div>
            {!isLast && <div className={`flex-1 h-[1px] mx-2 ${i < currentIndex ? 'bg-[#1877F2]' : 'bg-gray-200'}`} />}
          </div>
        )
      })}
    </div>
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 pb-6 border-b border-gray-100">
        <div>
          <h3 className="text-sm font-black uppercase text-[#050505] mb-0.5">
            {o.customerName || '--'}
            <span className="font-normal text-gray-500 ml-2">({o.company || 'No Company'})</span>
          </h3>
          <p className="text-[11px] text-gray-400">
            Sales Agent: <span className="text-[#1877F2] font-bold">{o.salesPerson || '--'}</span>
          </p>
          <p className="text-[11px] text-gray-400">
        ...
        <div className="flex flex-col items-end gap-2 w-full md:w-auto">
          ...
          <p className="text-[15px] font-black font-mono text-[#050505]">
          ...
          <div className="flex flex-col gap-3">
            {orderItems.map((item: any, i: number) => (
              <div key={i} className="flex items-baseline justify-between gap-2 text-[11px] text-gray-700">
                ...
                <span className="font-mono font-bold text-[#050505] shrink-0 text-right">
              ...
              orderPOs.map((po: any) => (
                ...
                >
                  <span className="text-gray-700 font-medium truncate">
                    <span className="font-mono font-bold text-[#050505] mr-2">{po.poNumber}</span>
                    {po.supplierName || 'Unnamed Supplier'}
                  </span>
                  <span className="text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 bg-[#1877F2]/10 text-[#1877F2] rounded flex-shrink-0">
                    {po.status}
                  </span>
                </Link>
              ))
      ...
      <div className="flex flex-col md:flex-row justify-end items-start md:items-end border-t border-gray-100 pt-6 gap-4 mt-2">
        <div className="bg-gray-50 p-4 md:p-5 rounded-xl flex flex-col gap-2.5 border border-gray-200 w-full md:w-[340px]">
          ...
          {o.paymentStatus === 'paid' && showPaymentMethod && (
              <div className="flex justify-between items-center text-[10px] text-[#1877F2]/70 bg-[#1877F2]/[0.04] border border-[#1877F2]/10 -mx-3 px-3 py-2 rounded-lg my-1">
                  <span>Mode of Payment</span>
                  <span className="font-semibold uppercase tracking-wide">{PAYMENT_METHOD_LABELS[o.paymentMethod] || o.paymentMethod}</span>
              </div>
          )}
          ...
          <div className="flex justify-between items-center text-[12px] font-bold text-[#1877F2] border-t border-gray-200 pt-2.5 mt-1">
    ...
    <>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full pl-9 pr-8 py-2 text-[12px] text-gray-700 placeholder:text-gray-300 bg-gray-50/70 border border-gray-100 rounded-lg focus:outline-none focus:border-[#1877F2] focus:bg-white transition-colors"
        />
        {searchQuery && (
          <button
            type="button"
