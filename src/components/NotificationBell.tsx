  ...
  return (
    <div className="relative" ref={panelRef}>
      <button
        ...
        className="relative flex items-center justify-center w-9 h-9 rounded-full hover:bg-[#050505]/[0.05] transition-colors"
      >
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 max-w-[85vw] bg-white border border-[#050505]/10 shadow-[0_20px_50px_-12px_rgba(1,23,47,0.25)] z-50">
          <div className="px-4 py-3 border-b border-[#050505]/10">
            <p className="text-[12px] font-black uppercase tracking-wide text-[#050505]">Notifications</p>
          </div>

          {items.length === 0 && !loadingMore ? (
            <p className="px-4 py-8 text-center text-[13px] text-[#050505]/40 font-medium">
            ...
            <div className="max-h-80 overflow-y-auto custom-scrollbar" onScroll={handleScroll}>
              {items.map((item) => (
                <Link
                  ...
                  className={`flex items-start gap-3 px-4 py-3 border-b border-[#050505]/5 hover:bg-[#f4f6f2] transition-colors ${!item.read ? 'bg-blue-50/20' : 'opacity-70'}`}
                >
                  <span className={`w-2 h-2 rounded-full flex-shrink-0 mt-1.5 ${!item.read ? 'bg-[#1877F2]' : 'bg-gray-300'}`} />
                  <div className="min-w-0 flex-1">
                    <p className={`text-[13px] text-[#050505] leading-snug break-words ${!item.read ? 'font-bold' : 'font-medium'}`}>{item.message}</p>
                    <p className="text-[11px] text-[#050505]/40 font-medium mt-0.5">{timeAgo(item.createdAt)}</p>
                  </div>
                  {!item.read && (
                    <span className="text-[10px] font-bold text-[#1877F2] ml-2 mt-0.5 flex-shrink-0">View</span>
                  )}
                </Link>
              ))}
              
              {loadingMore && (
                <div className="py-4 text-center text-[10px] text-[#050505]/40 font-bold uppercase tracking-widest">
                  Loading older...
                </div>
              )}
            </div>
          ...
          <Link
            ...
            className="block px-4 py-3 text-center text-[11px] font-bold uppercase tracking-wide text-[#3D5568] hover:text-[#1877F2] transition-colors border-t border-[#050505]/10"
          >
