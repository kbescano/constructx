  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white max-w-lg w-full max-h-[80vh] flex flex-col shadow-[0_30px_80px_-20px_rgba(1,23,47,0.35)]">
        <div className="p-6 border-b border-[#050505]/10">
          ...
          {loading ? (
            ...
            clients.map((c) => (
              <button
                key={c.id}
                onClick={() => onSelect(c)}
                className="w-full text-left px-6 py-4 hover:bg-[#1877F2]/[0.05] transition-colors border-b border-gray-100 last:border-0"
              >
                <p className="font-bold text-[#050505] text-sm">{c.name}</p>
        ...
        <div className="p-4 border-t border-[#050505]/10">
          <button
            onClick={onSkip}
            className="w-full py-3 text-sm font-bold uppercase tracking-wide text-gray-500 hover:text-[#050505] transition-colors"
          >
            Start Blank Instead
          </button>
        </div>
