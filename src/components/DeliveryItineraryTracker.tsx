...
const CONSTRUCTX_ADDRESS =
  "UNIT 4B, METRO LOGISTICS HUB, INDUSTRIAL AVE, BRGY. SAN ISIDRO, 4103 CAVITE";
const CONSTRUCTX_PHONE = "0917-000-0000 / 0917-000-0001";
...
export default function DeliveryItineraryTracker({
  ...
  const [constructXItems, setConstructXItems] = useState<CargoItem[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([fetchItin, fetchPOs, fetchOrder])
      .then(([itinData, poData, orderData]) => {
        ...
        const unassignedConstructX: CargoItem[] = [];
        ...
        masterItems.forEach((item: any) => {
          ...
          const cargoItem: CargoItem = {
            ...
            qty: itemQty,
            unit: itemUnit,
            sourceName: "ConstructX Trading (Warehouse)",
            sourcePO: "",
            sourceAddress: CONSTRUCTX_ADDRESS,
          };
          ...
          if (matchedPO) {
            ...
            unassignedConstructX.push(cargoItem);
          }
        ...
        setConstructXItems(unassignedConstructX);
        setLinkedPOs(poList);
        ...
        if (itinData.docs && itinData.docs.length > 0) {
          ...
          if (unassignedConstructX.length > 0) {
            autoStops.push({
              type: "pickup",
              address: CONSTRUCTX_ADDRESS,
              contactName: "ConstructX Trading (Warehouse)",
              contactPhone: CONSTRUCTX_PHONE,
              scheduledDate: null,
              status: "pending",
            });
          }
  ...
  function isConstructXStop(stop: RouteStop) {
    return (
      stop.contactName.toLowerCase().includes("constructx") ||
    ...
    return (
      <div className="mb-6 mx-5 md:mx-8 bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
        <div className="px-4 py-3 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
          <h4 className="text-[11px] font-bold uppercase tracking-widest text-[#050505]">
        ...
        {isSingleSource && (
          <div className="bg-[#1877F2]/5 px-4 py-2.5 border-b border-[#1877F2]/10 text-[10px] font-bold uppercase tracking-widest text-[#1877F2] flex items-center flex-wrap gap-1">
            📦 Source: {cargoItems[0].sourceName}
            {cargoItems[0].sourcePO && (
              <span className="bg-white border border-[#1877F2]/20 px-1.5 py-0.5 rounded mx-1 text-[#050505]">
                {cargoItems[0].sourcePO}
              </span>
            )}
  ...
  <tbody className="divide-y divide-gray-100">
    {cargoItems.map((item, idx) => {
      const isConstructX = item.sourceName === "ConstructX Trading (Warehouse)";
      return (
        <tr key={idx} className="hover:bg-gray-50/50">
          <td className="px-4 py-3 text-[12px] font-bold text-[#050505] align-top">
            {item.qty} <span className="text-[10px] text-gray-400 font-normal">{item.unit}</span>
          </td>
          <td className="px-4 py-3 align-top break-words">
            <div className="text-[12px] font-bold uppercase text-[#050505] leading-tight break-words">
              {item.description}
            </div>
            {!isSingleSource && (
              <div
  ...
  const WebPickupItemsDetails = ({
    ...
    const constructXStop = isConstructXStop(stop);
    if (!constructXStop && matchedPOs.length === 0) return null;

    return (
      <div className="mt-4 flex flex-col gap-3">
        {constructXStop && constructXItems.length > 0 && (
          <div className="bg-blue-50/50 border border-blue-100 rounded-lg p-3">
            <p className="text-[9px] font-bold uppercase tracking-widest text-blue-800 mb-2">
              ConstructX Inventory to Load:
            </p>
            <ul className="space-y-1 pl-1">
              {constructXItems.map((item, idx) => {
        ...
        {matchedPOs.length > 0 && (
          <div className="bg-[#1877F2]/5 border border-[#1877F2]/20 rounded-lg p-3">
            <p className="text-[9px] font-bold uppercase tracking-widest text-[#1877F2] mb-2">
              Supplier Items to Load:
            </p>
            <div className="flex flex-col gap-3">
              {matchedPOs.map((po, poIdx) => (
                <div key={poIdx}>
                  <span className="text-[9px] font-bold text-[#050505] border border-[#050505]/10 px-1.5 py-0.5 rounded mb-1.5 inline-block bg-white shadow-sm">
                    {po.poNumber}
                  </span>
                  <ul className="space-y-1 pl-1">
                    {po.items.map((item, idx) => {
                      ...
                      return (
                        <li
                          ...
                          className="text-[11px] text-gray-700 font-medium flex items-start gap-2"
                        >
                          <span className="font-bold text-[#1877F2] w-12 shrink-0">
                            {item.qty} {item.unit}
                          </span>
                          <span className="uppercase">
                            {item.description}{" "}
...
`}</style>
      ...
      {!itinerary || setupMode ? (
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-white p-5 border border-gray-200 rounded-xl shadow-sm">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-[13px] focus:outline-none focus:border-[#1877F2]"
                />
              </div>
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">
                ...
                >
                  ...
                  {stop.type === "pickup" && (
                    <div className="flex flex-wrap items-center gap-2 mb-4">
                      ...
                      <button
                        ...
                        className="text-[9px] font-bold uppercase tracking-widest px-2 py-1 bg-[#050505]/10 text-[#050505] hover:bg-[#050505] hover:text-white rounded transition-colors focus:outline-none"
                      >
                        ConstructX
                      </button>
                      ...
                      {uniqueSuppliersForQuickFill.map((po, idx) => (
                        <button
                          ...
                          className="text-[9px] font-bold uppercase tracking-widest px-2 py-1 bg-[#1877F2]/10 text-[#1877F2] hover:bg-[#1877F2] hover:text-white rounded transition-colors focus:outline-none"
                        >
                          {po.name}
                        </button>
                      ))}
                  ...
                  <div className="flex flex-col gap-3 mt-2">
                    <input
                      ...
                      required
                      className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-[13px] text-gray-900 focus:outline-none focus:border-[#1877F2] shadow-sm transition-colors"
                    />
                    ...
                    <div className="mt-2">
                      ...
                      <input
                        ...
                        className="w-full sm:w-[250px] px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-[12px] focus:outline-none focus:border-[#1877F2]"
                      />
                    </div>
                  </div>
            ...
            <div className="mt-4 flex flex-col sm:flex-row gap-3">
              <button
                ...
                className="relative flex items-center justify-center w-full flex-1 bg-white text-[#3D5568] py-3.5 overflow-hidden group cursor-pointer border-2 border-[#1877F2] rounded-lg disabled:opacity-50 transition-colors duration-300 shadow-sm hover:shadow-md"
              >
              ...
              <button
                ...
                className="flex items-center justify-center px-8 py-3.5 bg-[#050505] text-white text-[12px] font-bold uppercase tracking-[0.15em] rounded-lg hover:bg-[#0A4FB0] transition-colors shadow-sm focus:outline-none flex-shrink-0"
              >
                <svg
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden print:hidden">
          <div className="p-5 md:p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <span
                  className={`px-2.5 py-1 text-[9px] font-bold uppercase tracking-wider rounded ${itinerary.status === "delivered" ? "bg-[#1877F2]/10 text-[#1877F2]" : "bg-amber-50 text-amber-600"}`}
                >
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => window.print()}
                className="text-[10px] font-bold uppercase tracking-widest px-4 py-2 bg-[#1877F2] text-white hover:bg-[#0A4FB0] rounded-lg transition-colors shadow-sm focus:outline-none"
              >
                Print Waybill
              </button>
            </div>
          ...
          {(itinerary.driverName || itinerary.vehicleDetails) && (
            <div className="bg-gray-50 px-5 md:px-6 py-3 border-y border-gray-100 flex flex-wrap gap-x-8 gap-y-2 text-[11px] text-[#050505]">
              ...
              {itinerary.stops.map((stop, i) => {
                ...
                return (
                  <div key={stop.id || i} className="relative pl-6 md:pl-8">
                    <div
                      className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-4 border-white ${isCompleted ? "bg-[#1877F2]" : isArrived ? "bg-amber-400" : "bg-gray-200"}`}
                    />
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <h4
                          className={`text-[15px] font-medium leading-snug ${isCompleted ? "text-gray-400 line-through" : "text-[#050505]"}`}
                        >
                          {stop.address}
                        </h4>
                        ...
                        {stop.scheduledDate && (
                          <p className="text-[11px] text-[#1877F2] mt-1.5 font-bold flex items-center gap-1.5">
                            <svg
                              width="12"
                              height="12"
                              viewBox="0 0 24 24"
                      ...
                      {!isCompleted && (
                        <div className="flex items-center gap-2 shrink-0">
                          <select
                            ...
                            disabled={updating}
                            className="bg-white border border-gray-200 text-[#050505] text-[10px] font-bold uppercase tracking-widest rounded-lg px-3 py-2 outline-none focus:border-[#1877F2] transition-colors cursor-pointer disabled:opacity-50 shadow-sm"
                          >
      ...
      {/* ========================================== */}
      {/* HIDDEN PRINT DOCUMENT (CONSTRUCTX WAYBILL) */}
      {/* ========================================== */}
      <div className="waybill-print-doc hidden print:block w-full bg-white text-[#050505]">
        {/* Print Header */}
  ...
  <div className="flex gap-1.5 items-center">
    <div className="relative w-32 h-32 flex-shrink-0 overflow-hidden">
      <Image
        ...
        alt="ConstructX Trading Corp"
        fill
        className="object-contain scale-[1.1]"
      />
    </div>
    <div>
      <h2 className="text-lg font-black leading-none text-[#0A4FB0] tracking-tight">CONSTRUCTX</h2>
      <p className="text-[10px] font-bold tracking-[0.2em] text-[#050505] mt-0 mb-1.5">
        TRADING CORPORATION
      </p>
      <div className="w-full max-w-[220px] h-[2px] bg-[#1877F2] mb-1.5" />
      <p className="text-[8px] font-bold text-[#0A4FB0] leading-snug max-w-[220px] m-0">
        UNIT 4B, METRO LOGISTICS HUB, INDUSTRIAL AVE, BRGY. SAN ISIDRO, 4103 CAVITE,
        PHILIPPINES
      </p>
      <p className="text-[8px] font-bold text-[#0A4FB0] m-0 mt-1">
        0917-000-0000 / 0917-000-0001
      </p>
      <p className="text-[8px] font-bold text-[#0A4FB0] m-0 mt-1">SALES@CONSTRUCTX.DEMO</p>
    </div>
  </div>

  <div className="text-right w-auto">
    <h3 className="text-lg font-bold text-[#3D5568] mb-1">
      DELIVERY WAYBILL
    </h3>
    <table className="text-xs ml-auto mt-0">
      <tbody>
        ...
        <div className="mb-4">
          <div className="bg-[#3D5568] text-white text-[9px] font-bold uppercase tracking-widest px-2 py-1 mb-2">
              ...
              {(() => {
                ...
                return (
                  <div className="w-full border border-gray-200 border-t-0">
                    {isSingleSource && (
                      <div className="bg-gray-100/80 px-2 py-1.5 border-t border-gray-200 text-[8px] font-bold uppercase tracking-widest text-[#050505]">
                        Source: {cargoItems[0].sourceName}{" "}
                        {cargoItems[0].sourcePO &&
                    ...
                    <table className="w-full border-collapse">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {cargoItems.map((item, idx) => {
                          const isConstructX =
                            item.sourceName === "ConstructX Trading (Warehouse)";
                          return (
                            <tr key={idx} className="border-b border-gray-100">
                              ...
                              <td className="py-2 align-top pr-4">
                                ...
                                {!isSingleSource && (
                                  <div
                                    className={`mt-1 text-[7px] font-bold uppercase tracking-wider ${isConstructX ? "text-blue-600" : "text-gray-500"}`}
                                  >
                                    {item.sourceName}{" "}
                  ...
                  <tr key={i} className="border-b border-gray-200">
                    ...
                    <td className="py-3 align-top pr-4">
                      ...
                      <div className="flex flex-col gap-0.5 mb-1.5">
                        ...
                        {stop.scheduledDate && (
                          <p className="text-[9px] text-[#1877F2] font-bold mt-1">
                            Sched:{" "}
                            {new Date(stop.scheduledDate).toLocaleString(
                              "en-PH",
                      ...
                      {stop.type === "pickup" &&
                        constructXStop &&
                        constructXItems.length > 0 && (
                          <div className="mt-2 pl-2 border-l-2 border-[#050505]/30">
                            <p className="text-[8px] font-bold uppercase tracking-widest text-[#050505] mb-0.5">
                              ConstructX Inventory:
                            </p>
                      ...
                      {stop.type === "pickup" && matchedPOs.length > 0 && (
                        <div className="mt-2 pl-2 border-l-2 border-[#1877F2]/30">
                          <p className="text-[8px] font-bold uppercase tracking-widest text-[#1877F2] mb-0.5">
                            Supplier Items:
                          </p>
                          <div className="flex flex-col gap-1.5">
                            {matchedPOs.map((po, poIdx) => (
                              <div key={poIdx}>
                                <span className="text-[8px] font-bold text-[#050505] border border-[#050505]/20 px-1 rounded inline-block bg-gray-50 mb-0.5">
                                  {po.poNumber}
                                </span>
        ...
        <div className="text-[9px] text-gray-500 border-t border-gray-200 pt-2 text-center mt-6">
          <p className="font-bold uppercase tracking-widest text-[#050505] mb-0.5">
            Important Note
          </p>
          <p>
